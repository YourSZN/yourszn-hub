...
var NAV = [
  ...
  { id:'notifications', lbl:'Notifications', sec:null,   icon:'<path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/>' },
  { id:'calendar',  lbl:'Calendar',     sec:'Operations',  icon:'<rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>' },
...
function launchApp() {
  ...
  buildNav(u.pages);
  // Show a subtle loading indicator while cloud data loads
  var loadingBanner = document.createElement('div');
  loadingBanner.id = 'yszn-loading-banner';
  loadingBanner.style.cssText = 'position:fixed;top:0;left:220px;right:0;background:#F7F3EE;border-bottom:1px solid #E5DDD4;padding:10px 24px;font-size:12px;color:#8A7D72;letter-spacing:1px;text-transform:uppercase;z-index:999;text-align:center';
  loadingBanner.textContent = 'Loading your data...';
  document.body.appendChild(loadingBanner);

  loadData(); // async — _applyLoadedData fires when ready

  // Render initial UI with default/empty state immediately
  updateTaskBadge();
  try { updateOcaRevisedBadge(); } catch(e){}
  commsNavBadge();
  try { notifInit(); } catch(e){}
  try { myhubLoadTodos(); } catch(e){}
  showPage(u.pages[0]);
  renderClients();
  renderSops();
  renderAudit();
  ...
  // Loading banner is now removed by _applyLoadedData when cloud data arrives
}
...
function buildNav(allowed) {
  ...
  NAV.forEach(function(item) {
    ...
    btn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:18px;height:18px;flex-shrink:0">'+item.icon+'</svg><span class="nav-lbl">'+item.lbl+'</span>';
    if (item.id==='comms') {
    ...
    if (item.id==='clients') {
      var cbdg = document.createElement('span');
      cbdg.id = 'n-clients-badge';
      cbdg.style.cssText = 'display:none;background:var(--rose);color:#fff;border-radius:10px;padding:1px 7px;font-size:10px;font-weight:700;margin-left:auto;min-width:18px;text-align:center;';
      cbdg.textContent = '0';
      btn.appendChild(cbdg);
    }
...
// Debounced save — waits 1.5s after last keystroke before saving to cloud
var _saveTimer = null;
...
var taskWeekState = {};   // weekKey -> { taskId -> {status, hrsTaken, staffNotes} }
...
function getTS(t, off) {
  ...
  var wk = weekKey(off);
  if (!taskWeekState[wk]) taskWeekState[wk] = {};
  var s = taskWeekState[wk][t.id];
  return s || {status:'not-started', hrsTaken:0, staffNotes:''};
}
function setTS(t, off, field, val) {
  if (t.freq !== 'daily' && t.freq !== 'weekly') { t[field] = val; return; }
  var wk = weekKey(off);
  if (!taskWeekState[wk]) taskWeekState[wk] = {};
...
// v37: shorthand — get the effective status for a task in "this week" (week 0) context
// Use this everywhere that needs current-week status for progress bars, counts, badges etc.
...
function buildOwnerPersonCard(uid) {
  ...
  var tot=myTasks.length, dn=myTasks.filter(function(t){return taskStat(t)==='done';}).length;
...
function buildTaskTablesHTML(uid, weekOff, isStaff) {
 ...
 var allWeekTasks = tasksForWeek(weekOff).filter(function(t){ 
  if ((t.assignedTo||t.assigned_to)!==uid) return false;
  ...
  // ── HOURS TALLY (only on Latisha's dashboard view) ──
  if (!isStaff) {
    var totalAllowed = 0, totalTaken = 0;
    allWeekTasks.forEach(function(t) {
      totalAllowed += parseFloat(t.hrsAllowed) || 0;
      totalTaken   += parseFloat(getTS(t, weekOff).hrsTaken) || 0;
    });
    var tallyColor = totalTaken > totalAllowed && totalAllowed > 0
      ? '#EF4444'
      : totalTaken >= totalAllowed && totalAllowed > 0
        ? '#22C55E'
        : 'var(--charcoal)';
    html += '<div style="display:flex;justify-content:flex-end;gap:24px;padding:14px 20px;margin-top:8px;background:var(--warm);border-radius:10px;border:1px solid var(--sand)">';
    html += '<div style="font-size:12px;color:var(--muted);display:flex;align-items:center;gap:6px"><span style="text-transform:uppercase;letter-spacing:.8px;font-weight:600">Total Hours Allowed</span> <span style="font-size:16px;font-weight:700;color:var(--deep)">' + (totalAllowed ? totalAllowed + 'h' : '—') + '</span></div>';
    html += '<div style="font-size:12px;color:var(--muted);display:flex;align-items:center;gap:6px"><span style="text-transform:uppercase;letter-spacing:.8px;font-weight:600">Total Hours Taken</span> <span style="font-size:16px;font-weight:700;color:' + tallyColor + '">' + (totalTaken ? totalTaken + 'h' : '—') + '</span></div>';
    html += '</div>';
  }

  if (!isStaff) {
    html += '<button class="btn btnp" style="margin-top:4px" onclick="openNewTaskModal(\''+uid+'\')">+ New Task</button>';
  }
  html += '</div>';
  return html;
}
...
function openNewTaskModal(assignTo) {
  editingTaskId = null;
  document.getElementById('modal-title').textContent = 'New Task';
  document.getElementById('modal-task-id').value = '';
  document.getElementById('mt-title').value = '';
  document.getElementById('mt-assign').value = assignTo || ((curUser === 'latisha') ? 'latisha' : curUser);
...
function openTaskModal(id) {
  ...
  document.getElementById('mt-status').value = getTS(t, taskWeekOff).status||'not-started';
...
async function saveTask() {
  ...
  if (editingTaskId) {
    ...
    if (updated && (updated.freq === 'daily' || updated.freq === 'weekly')) {
...
function openStaffTaskModal(id) {
  var t = tasks.find(function(x){ return x.id===id; }); if (!t) return;
  // Latisha uses full task modal as overlay (no page navigation)
  if (curUser === 'latisha') {
    editingTaskId = id;
    document.getElementById('modal-title').textContent = 'Edit Task';
    document.getElementById('modal-task-id').value = id;
...
var _s = getTS(t, staffTaskWeekOff);
  document.getElementById('stm-status').value = _s.status||'not-started';
  document.getElementById('stm-hrs').value = _s.hrsTaken||'';
  document.getElementById('stm-notes').value = _s.staffNotes||'';
...
function makeStaffCard(uid, showTaskBtn) {
  ...
  var wtTot=wt.length;
  var wtDn=wt.filter(function(t){return taskStat(t)==='done';}).length;
  var wtIp=wt.filter(function(t){var s=taskStat(t);return s==='in-progress'||s==='waiting';}).length;
...
function renderDashTaskProgress(){
  ...
  if(tot2>0){
    ...
    var ip2=all.filter(function(t){var s=taskStat(t);return s==='in-progress'||s==='waiting';}).length;
...
var _s=getTS(t, staffTaskWeekOff);
  document.getElementById('smodal-status').value=_s.status||'not-started';
  document.getElementById('smodal-hrs').value=_s.hrsTaken||'';
  document.getElementById('smodal-notes').value=_s.staffNotes||'';
...
function renderMyHub() {
  ...
  var dn  = myTasks.filter(function(t){ return taskStat(t)==='done'; }).length;
  var ip  = myTasks.filter(function(t){ var s=taskStat(t); return s==='in-progress'||s==='waiting'; }).length;
  var ns  = tot - dn - ip;
...
// ── Priority To-Dos (Supabase-backed) ──────────────────────
var myhubTodos = [];
var myhubTodosLoaded = false;

async function myhubLoadTodos() {
  var db = getSupa(); if (!db) return;
  try {
    var res = await db.from('priority_todos').select('*').eq('user_id', curUser).order('sort_order', { ascending: true }).order('created_at', { ascending: true });
    if (res.error) throw res.error;
    myhubTodos = res.data || [];
    myhubTodosLoaded = true;
    renderMyhubTodos();
  } catch(e) { console.warn('Failed to load todos:', e); }
}
...
function renderMyhubTodos() {
  ...
  var pending = myhubTodos.filter(function(t){ return !t.done; }).length;
  if (badge) badge.textContent = pending || '';
  if (!myhubTodosLoaded) { el.innerHTML='<div style="color:var(--muted);font-size:13px;padding:6px 0">Loading…</div>'; return; }
  if (!myhubTodos.length) { el.innerHTML='<div style="color:var(--muted);font-size:13px;padding:6px 0">No to-dos yet. Add one below.</div>'; return; }
  el.innerHTML = '';
  myhubTodos.forEach(function(item, idx) {
    var div = document.createElement('div');
    div.className = 'slist-item';
    div.style.cssText = 'display:flex;align-items:center;gap:6px;padding:6px 4px;border-bottom:1px solid var(--warm);';
    var circle = document.createElement('div');
    circle.className = 'slist-circle' + (item.done ? ' done' : '');
    circle.style.cssText = 'flex-shrink:0;cursor:pointer;';
    circle.onclick = function(e) { e.stopPropagation(); myhubTogTodo(item.id, !item.done); };
    div.appendChild(circle);
    var textEl = document.createElement('div');
    textEl.className = 'slist-text' + (item.done ? ' done' : '');
    textEl.style.cssText = 'flex:1;min-width:0;font-size:13px;cursor:text;';
    textEl.textContent = item.text;
    textEl.ondblclick = function(e) {
      e.stopPropagation(); textEl.contentEditable = 'true';
      textEl.style.outline = '1px solid var(--rose)'; textEl.style.borderRadius = '4px'; textEl.style.padding = '2px 4px';
      textEl.focus();
      var range = document.createRange(); range.selectNodeContents(textEl);
      var sel = window.getSelection(); sel.removeAllRanges(); sel.addRange(range);
    };
    textEl.onblur = function() {
      textEl.contentEditable = 'false'; textEl.style.outline = ''; textEl.style.padding = '';
      var newText = textEl.textContent.trim();
      if (newText && newText !== item.text) myhubUpdateTodo(item.id, { text: newText });
    };
    textEl.onkeydown = function(e) { if (e.key === 'Enter') { e.preventDefault(); textEl.blur(); } if (e.key === 'Escape') { textEl.textContent = item.text; textEl.blur(); } };
    div.appendChild(textEl);
    var tagSel = document.createElement('select');
    tagSel.style.cssText = 'font-size:10px;padding:2px 4px;border-radius:6px;border:1px solid #ddd;cursor:pointer;font-weight:700;flex-shrink:0;';
    var tagOpts = [{value:'',label:'—',bg:'#f4f4f4',color:'#888'},{value:'Urgent',label:'URGENT',bg:'#EF4444',color:'#fff'},{value:'Soon',label:'SOON',bg:'#F97316',color:'#fff'}];
    tagOpts.forEach(function(o) { var opt = document.createElement('option'); opt.value = o.value; opt.textContent = o.label; if ((item.tag||'') === o.value) opt.selected = true; tagSel.appendChild(opt); });
    var curTag = tagOpts.find(function(o){ return o.value === (item.tag||''); }) || tagOpts[0];
    tagSel.style.background = curTag.bg; tagSel.style.color = curTag.color;
    tagSel.onchange = function(e) { e.stopPropagation(); myhubUpdateTodo(item.id, {tag: tagSel.value||null}); var t = tagOpts.find(function(o){return o.value===tagSel.value;})||tagOpts[0]; tagSel.style.background=t.bg; tagSel.style.color=t.color; };
    tagSel.onclick = function(e) { e.stopPropagation(); };
    div.appendChild(tagSel);
    if (idx > 0) { var upBtn = document.createElement('button'); upBtn.innerHTML='▲'; upBtn.title='Move up'; upBtn.style.cssText='background:none;border:none;cursor:pointer;color:var(--muted);font-size:10px;padding:0 2px;flex-shrink:0;'; upBtn.onclick=function(e){e.stopPropagation();myhubReorderTodo(idx,idx-1);}; div.appendChild(upBtn); }
    if (idx < myhubTodos.length-1) { var dnBtn = document.createElement('button'); dnBtn.innerHTML='▼'; dnBtn.title='Move down'; dnBtn.style.cssText='background:none;border:none;cursor:pointer;color:var(--muted);font-size:10px;padding:0 2px;flex-shrink:0;'; dnBtn.onclick=function(e){e.stopPropagation();myhubReorderTodo(idx,idx+1);}; div.appendChild(dnBtn); }
    var delBtn = document.createElement('button'); delBtn.innerHTML='×'; delBtn.title='Delete'; delBtn.style.cssText='background:none;border:none;cursor:pointer;color:#C4A882;font-size:16px;padding:0 4px;flex-shrink:0;'; delBtn.onclick=function(e){e.stopPropagation();myhubDelTodo(item.id);}; div.appendChild(delBtn);
    el.appendChild(div);
  });
}
async function myhubAddTodo() {
  var inp = document.getElementById('myhub-todo-inp'); if(!inp||!inp.value.trim()) return;
  var tagSel = document.getElementById('myhub-todo-tag'); var tag = tagSel ? tagSel.value : '';
  var db = getSupa(); if (!db) return;
  var maxSort = myhubTodos.length > 0 ? Math.max.apply(null, myhubTodos.map(function(t){ return t.sort_order||0; })) + 1 : 0;
  try { var res = await db.from('priority_todos').insert({user_id:curUser,text:inp.value.trim(),tag:tag||null,done:false,sort_order:maxSort}).select().single(); if(res.error)throw res.error; myhubTodos.push(res.data); inp.value=''; if(tagSel)tagSel.value=''; renderMyhubTodos(); } catch(e) { console.warn('Failed to add todo:',e); alert('Failed to add to-do.'); }
}
async function myhubTogTodo(id, done) {
  var db = getSupa(); if (!db) return;
  try { await db.from('priority_todos').update({done:done,updated_at:new Date().toISOString()}).eq('id',id); var item=myhubTodos.find(function(t){return t.id===id;}); if(item)item.done=done; renderMyhubTodos(); } catch(e) { console.warn('Failed to toggle todo:',e); }
}
async function myhubUpdateTodo(id, updates) {
  var db = getSupa(); if (!db) return; updates.updated_at = new Date().toISOString();
  try { await db.from('priority_todos').update(updates).eq('id',id); var item=myhubTodos.find(function(t){return t.id===id;}); if(item){for(var k in updates)item[k]=updates[k];} renderMyhubTodos(); } catch(e) { console.warn('Failed to update todo:',e); }
}
async function myhubDelTodo(id) {
  if (!confirm('Delete this to-do?')) return;
  var db = getSupa(); if (!db) return;
  try { await db.from('priority_todos').delete().eq('id',id); myhubTodos=myhubTodos.filter(function(t){return t.id!==id;}); renderMyhubTodos(); } catch(e) { console.warn('Failed to delete todo:',e); }
}
async function myhubReorderTodo(fromIdx, toIdx) {
  var item = myhubTodos.splice(fromIdx,1)[0]; myhubTodos.splice(toIdx,0,item);
  var db = getSupa(); if (!db) return;
  try { for(var i=0;i<myhubTodos.length;i++){myhubTodos[i].sort_order=i; await db.from('priority_todos').update({sort_order:i}).eq('id',myhubTodos[i].id);} renderMyhubTodos(); } catch(e) { console.warn('Failed to reorder:',e); }
}
...
function renderMyhubRecur() {
  ...
  recur.forEach(function(t) {
    ...
    row.innerHTML =
      ...
      ' <span class="tcard-status st-'+(taskStat(t)||'not-started')+'" style="font-size:9px;vertical-align:middle">'+statusLabel(taskStat(t))+'</span></div>'+
...
function renderMyhubTeam() {
  ...
  others.forEach(function(oid){
    ...
    var wtTot=wt.length, wtDn=wt.filter(function(t){return taskStat(t)==='done';}).length;
    var wtIp=wt.filter(function(t){var s=taskStat(t);return s==='in-progress'||s==='waiting';}).length;
    var wtNs=wtTot-wtDn-wtIp;
    var pw=function(n){return wtTot>0?(n/wtTot*100).toFixed(1)+'%':'0%';};
...
function saveBrand(){
  ...
  renderBrands(); showHsTab('brands'); saveData();
}
...
function delWatch(id){ watchlist=watchlist.filter(function(w){return w.id!==id;}); renderWatchlist(); saveData(); }
...
function openHideModal(taskId) {
  ...
  var _s = getTS(t, _hoff);
  document.getElementById('hm-task-id').value = taskId;
  document.getElementById('hm-title').textContent = t.title;
  document.getElementById('hm-status').value = _s.status === 'done' ? 'done' : _s.status || 'done';
  document.getElementById('hm-hrs').value = _s.hrsTaken || '';
  document.getElementById('hm-notes').value = _s.staffNotes || '';
...
function confirmHideTask() {
  ...
  var t = tasks.find(function(x){ return x.id===taskId; }); if (!t) return;
  var _hoff = (curUser === 'latisha') ? taskWeekOff : staffTaskWeekOff;
  setTS(t, _hoff, 'status', status);
  setTS(t, _hoff, 'hrsTaken', parseFloat(hrs) || 0);
  setTS(t, _hoff, 'staffNotes', notes);
...
else if (vtTab === 'lookbook') {
    panelHtml = '<iframe src="/lookbook.html" style="width:100%;min-height:85vh;border:none;border-radius:10px"></iframe>';
  }
...
function saveData() {
  if (!window._cloudDataLoaded) {
    console.warn('YourSZN: save blocked — cloud data not loaded yet');
    return;
  }
  try {
...
function _applyLoadedData(d) {
  ...
  if (d.tours)             tours             = d.tours;
  if (d.tasks)             tasks             = d.tasks;
  // ── Normalize task field names (snake_case → camelCase) ──
  if (tasks && tasks.length) {
    tasks.forEach(function(t) {
      if (t.hrs_allowed !== undefined && t.hrsAllowed === undefined) { t.hrsAllowed = t.hrs_allowed; }
      if (t.hrs_taken !== undefined && t.hrsTaken === undefined)     { t.hrsTaken = t.hrs_taken; }
      if (t.due_date !== undefined && t.due === undefined)           { t.due = t.due_date; }
      if (t.week_date !== undefined && t.weekDate === undefined)     { t.weekDate = t.week_date; }
      if (t.video_url !== undefined && t.videoUrl === undefined)     { t.videoUrl = t.video_url; }
      if (t.file_url !== undefined && t.fileUrl === undefined)       { t.fileUrl = t.file_url; }
      if (t.description !== undefined && t.desc === undefined)       { t.desc = t.description; }
      if (t.staff_notes !== undefined && t.staffNotes === undefined) { t.staffNotes = t.staff_notes; }
    });
  }
  // ── AUTO-RESET hrsTaken every Monday ──
  var now = new Date();
...
if (lastReset !== thisMondayStr && tasks && tasks.length) {
    ...
    hiddenTasks = {};
    taskWeekState = {}; // v37: clear per-week state on weekly reset
  ...
  try { renderCreators(); } catch(e){}
  try { renderMarketing(); } catch(e){}
  try { renderVietnamTour(); } catch(e){}
  try { renderCelebList(); } catch(e){}
  try { commsNavBadge(); } catch(e){}
  try { renderAudit(); } catch(e){}
  try { renderBrands(); } catch(e){}
  try { renderWatchlist(); } catch(e){}
  try { renderAdCreativePage(); } catch(e){}
  ...
  if (curUser === 'latisha') {
    ...
    try { renderMyHub(); } catch(e){}
  }
  // Mark cloud data as loaded — saves are now allowed
  window._cloudDataLoaded = true;
  // Remove loading banner
  var b = document.getElementById('yszn-loading-banner');
  if (b) b.remove();
}
...
var MRC_STAFF_COLORS = {L:'#7A8C6E', S:'#C49A8A', T:'#C4956A', '':'#C9B99A'};
var MRC_STAFF_LABELS = {L:'Lemari', S:'Salma', T:'Latisha', '':'Unassigned'};
...
function openMrcModal(key) {
  ...
  if (slot.staff) { badge.textContent = slot.staff; badge.style.background = staffColors[slot.staff]||'#C9B99A'; badge.style.display='flex'; }
  else            { badge.style.display='none'; }
  var linksHtml = '';
  if (slot.contentType) linksHtml += '<div style="margin-bottom:10px"><span style="font-size:10px;font-weight:700;text-transform:uppercase;color:var(--muted)">Type: </span><span style="font-size:13px;color:var(--charcoal)">'+slot.contentType+'</span></div>';
  if (slot.canvaWip)    linksHtml += '<a href="'+slot.canvaWip+'"  target="_blank" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:#6D28D9;background:#F5F3FF;padding:7px 14px;border-radius:8px;border:1px solid #DDD6FE;text-decoration:none;margin-right:8px;margin-bottom:8px">&#127912; Canva \u2014 WIP</a>';
  if (slot.canvaDone)   linksHtml += '<a href="'+slot.canvaDone+'" target="_blank" style="display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:#065F46;background:#D1FAE5;padding:7px 14px;border-radius:8px;border:1px solid #A7F3D0;text-decoration:none;margin-bottom:8px">&#127912; Canva \u2014 Final</a>';
  if (!linksHtml) linksHtml = '<div style="font-size:12px;color:var(--muted)">Empty slot \u2014 click Edit to fill in.</div>';
  document.getElementById('mrc-links-view').innerHTML = linksHtml;
  var notesHtml = slot.notes ? '<div style="font-size:13px;color:var(--muted);font-style:italic;line-height:1.6;white-space:pre-wrap">'+slot.notes+'</div>' : '';
  document.getElementById('mrc-notes-view').innerHTML = notesHtml;
  var thumbView = document.getElementById('mrc-thumb-view');
  if (thumbView) thumbView.innerHTML = slot.thumb ? '<img src="'+slot.thumb+'" style="width:100%;max-height:200px;object-fit:cover;border-radius:10px;border:1px solid var(--sand)">' : '';
...
function switchMrcToEdit() {
  ...
  if (mrcPendingThumb) { prev.src = mrcPendingThumb; prev.style.display='block'; if(clrBtn) clrBtn.style.display='block'; }
  else                 { prev.style.display='none'; if(clrBtn) clrBtn.style.display='none'; }
...
function saveMrcSlot() {
  ...
  saveData(); closeMrcModal(); renderMetaSchedule();
}

function clearMrcSlot() {
  if (currentMrcKey) { delete metaSchedData[currentMrcKey]; saveData(); }
  closeMrcModal(); renderMetaSchedule();
}
...
function handleMrcThumb(inp) {
  ...
  reader.onload = function(e) {
    ...
    var prev = document.getElementById('mrc-thumb-prev');
    prev.src = mrcPendingThumb; prev.style.display = 'block';
...
function openCelebVidModal(cid, vid) {
  ...
  var c = celebData.find(function(x){ return x.id === cid; });
  if (!c) return;
  var v = vid ? (c.videos||[]).find(function(x){ return x.id === vid; }) : null;

  document.getElementById('cvm-heading').textContent = v ? 'Edit Video' : 'Add Video';
  document.getElementById('cvm-celeb-name').textContent = c.name;

  // Category radios
  var catVal = v ? (v.cat || 'celeb') : 'celeb';
  document.querySelectorAll('input[name="cvm-cat-r"]').forEach(function(r){ r.checked = r.value === catVal; });
  ...
  document.getElementById('cvm-editor').value    = v ? (v.editor || 'lemari') : 'lemari';
  document.getElementById('cvm-caption').value   = v ? (v.caption|| '') : '';
  document.getElementById('cvm-cta').value       = v ? (v.cta    || '') : '';
  document.getElementById('cvm-hashtags').value  = v ? (v.hashtags||'#colouranalysis #yourszn') : '#colouranalysis #yourszn';
  document.getElementById('cvm-status').value    = v ? (v.status || 'editing') : 'editing';
  document.getElementById('cvm-link').value      = v ? (v.link   || '') : '';
  document.getElementById('cvm-notes').value     = v ? (v.notes  || '') : '';
  document.getElementById('cvm-date-completed').value = v ? (v.dateCompleted || '') : '';
  document.getElementById('cvm-date-published').value = v ? (v.datePublished || '') : '';

  celebVidThumb = v ? (v.thumb || null) : null;
  var prev = document.getElementById('cvm-thumb-prev');
  if (celebVidThumb) { prev.src = celebVidThumb; prev.style.display = 'block'; }
  else               { prev.src = ''; prev.style.display = 'none'; }
  var finp = document.getElementById('cvm-thumb-input');
  if (finp) finp.value = '';

  document.getElementById('cvm-del').style.display = v ? 'inline-block' : 'none';
...
function closeCelebVidModal() {
  ...
  celebVidEditCid = null; celebVidEditId = null; celebVidThumb = null;
}
...
function handleCvmThumb(inp) {
  ...
  reader.onload = function(e) {
    ...
    var prev = document.getElementById('cvm-thumb-prev');
    prev.src = celebVidThumb; prev.style.display = 'block';
  };
  reader.readAsDataURL(inp.files[0]);
}

function saveCelebVid() {
  var c = celebData.find(function(x){ return x.id === celebVidEditCid; });
  if (!c) return;
  ...
  if (celebVidEditId) {
    var idx = c.videos.findIndex(function(x){ return x.id === celebVidEditId; });
    if (idx > -1) c.videos[idx] = vidObj; else c.videos.push(vidObj);
  } else {
    c.videos.push(vidObj);
  }
  saveData(); closeCelebVidModal(); renderCelebList();
}

function deleteCelebVid() {
  var c = celebData.find(function(x){ return x.id === celebVidEditCid; });
  if (!c || !celebVidEditId) return;
  c.videos = (c.videos||[]).filter(function(v){ return v.id !== celebVidEditId; });
  saveData(); closeCelebVidModal(); renderCelebList();
}
...
var REPORT_STRIPES = {"Bright Spring": ["#fe2509", "#582d23", "#5b21aa", "#d55942", "#cb7131", "#c8581e", "#d87b3d", "#f53744", "#7b1ccd", "#25388f", "#65cbdd", "#4fc3e7", "#b1e956", "#3ceb0b", "#bff541", "#bcbb98", "#ed8a79", "#bd5015", "#a62bb9", "#dfa854"], "Bright Winter": ["#8f03cd", "#71c9f6", "#543efb", "#260587", "#ac012e", "#a9b022", "#7e9960", "#23a950", "#2d3b7b", "#cd36ad", "#5abbc5", "#a8584b", "#480304", "#a6bd3e", "#f54eb1", "#5023e5", "#011964", "#b92f13", "#fbce45", "#fb5a6f"], "Dark Autumn": ["#fab959", "#2e3e3a", "#2289a1", "#2f3430", "#5b1e03", "#e4c195", "#c4551d", "#e2775e", "#86170f", "#7a0172", "#3612a0", "#5f5f28", "#23310d", "#433e03", "#d8b34d", "#70634f", "#d46b7b", "#3e3c5b", "#902c3f", "#431c12"], "Dark Winter": ["#b00019", "#40211e", "#7d422e", "#543a65", "#d343a2", "#7b2fa0", "#8919c3", "#5b37be", "#2a236d", "#187fd8", "#05dcf6", "#029066", "#023f1e", "#c0ca50", "#d8dbb8", "#4380a5", "#ae444b", "#463d3c", "#b93fca"], "Light Spring": ["#84b6d1", "#c38b66", "#ea787e", "#d9a88d", "#b1895f", "#e1956a", "#f05f72", "#9674cf", "#ad75dd", "#22d7ef", "#01b796", "#56afd9", "#6ebc75", "#cfdb70", "#b2b094", "#ea8877", "#f1c3ab", "#7cc4cd", "#d3e36b"], "Light Summer": ["#e394d8", "#cae18f", "#4ce7cc", "#b6e79a", "#9a95cb", "#dc90a9", "#8378c1", "#c090d1", "#9e7ac8", "#b5a1a8", "#7e95ca", "#79aecb", "#8ed7c1", "#8bce80", "#e1ef7d", "#cbcbb2", "#de5a6c", "#cd617e", "#ae8c86", "#7c96cf"], "Soft Autumn": ["#37122f", "#203e61", "#2f7d7b", "#3c4740", "#baab86", "#706050", "#a35034", "#c77871", "#816593", "#3a6372", "#888d49", "#545d38", "#798b5b", "#dfcb78", "#80735a", "#80333e", "#40304c", "#615045", "#d2a59a"], "Soft Summer": ["#ce899e", "#df5d72", "#a26d6d", "#674a40", "#87212a", "#8b275e", "#8c2f88", "#5d2a80", "#29165c", "#39377f", "#4d62a7", "#4888b0", "#356584", "#479691", "#228677", "#adc581", "#636277", "#63364c", "#d8859e", "#71485c"], "True Autumn": ["#c97a41", "#624527", "#54341d", "#884d22", "#d64a1f", "#a7321c", "#cd753f", "#cd5f23", "#dd8460", "#ad4132", "#5c4270", "#8c8553", "#586039", "#788e42", "#c49c4c", "#7f6c51", "#d77965", "#784d5a", "#ba4d4e", "#f49b44"], "True Spring": ["#c97a42", "#9f5b28", "#a37349", "#cb6e24", "#e54d1e", "#a4321d", "#db485c", "#e7718a", "#7753b1", "#3f5483", "#10c7d2", "#80d076", "#5b8543", "#92b450", "#e9b743", "#b5a586", "#e48b79", "#984c29", "#934aa6", "#e0be56"], "True Summer": ["#4aa167", "#4db6c5", "#8c9e9d", "#69554e", "#ac263e", "#d280b6", "#945a8d", "#4d107c", "#5137ac", "#5369b5", "#6fa3ed", "#55a7ab", "#7ac17a", "#dded7d", "#cccbb2", "#7695d8", "#b9609c", "#e586c7", "#9283e6"], "True Winter": ["#a8c1f7", "#d33679", "#f0bc59", "#97f28f", "#193122", "#4d0729", "#c50b84", "#a613c1", "#e36ce5", "#5954e7", "#1caeda", "#079a63", "#b7da6e", "#3e497b", "#01041d", "#a00a23", "#4a68bb", "#392ae1", "#a517e2", "#4d32ee"]};
var REPORT_PATTERNS = {"spring": "data:image/png;base64,", "summer": "data:image/png;base64,", "winter": "data:image/png;base64,", "autumn": "data:image/png;base64,"};
var REPORT_SEASON_GROUP = {"Light Spring": "spring", "True Spring": "spring", "Bright Spring": "spring", "Light Summer": "summer", "True Summer": "summer", "Soft Summer": "summer", "Soft Autumn": "autumn", "True Autumn": "autumn", "Dark Autumn": "autumn", "True Winter": "winter", "Dark Winter": "winter", "Bright Winter": "winter"};
var REPORT_SEASON_DESC = {"Light Spring": {"tone": "Cool & Light", "undertone": "Warm", "contrast": "Low to Medium"}, "True Spring": {"tone": "Warm & Clear", "undertone": "Warm", "contrast": "Medium"}, "Bright Spring": {"tone": "Warm & Bright", "undertone": "Warm", "contrast": "High"}, "Light Summer": {"tone": "Cool & Light", "undertone": "Cool", "contrast": "Low"}, "True Summer": {"tone": "Cool & Muted", "undertone": "Cool", "contrast": "Low to Medium"}, "Soft Summer": {"tone": "Cool & Soft", "undertone": "Cool", "contrast": "Low"}, "Soft Autumn": {"tone": "Warm & Soft", "undertone": "Warm", "contrast": "Low to Medium"}, "True Autumn": {"tone": "Warm & Muted", "undertone": "Warm", "contrast": "Medium"}, "Dark Autumn": {"tone": "Warm & Dark", "undertone": "Warm", "contrast": "High"}, "True Winter": {"tone": "Cool & Clear", "undertone": "Cool", "contrast": "High"}, "Dark Winter": {"tone": "Cool & Dark", "undertone": "Cool", "contrast": "High"}, "Bright Winter": {"tone": "Cool & Bright", "undertone": "Cool", "contrast": "High"}};
var REPORT_SISTERS = {"Light Spring": ["True Spring", "Light Summer"], "True Spring": ["Bright Spring", "Light Spring"], "Bright Spring": ["True Spring", "Bright Winter"], "Light Summer": ["True Summer", "Light Spring"], "True Summer": ["Soft Summer", "Light Summer"], "Soft Summer": ["True Summer", "Soft Autumn"], "Soft Autumn": ["True Autumn", "Soft Summer"], "True Autumn": ["Soft Autumn", "Dark Autumn"], "Dark Autumn": ["True Autumn", "Dark Winter"], "True Winter": ["Bright Winter", "True Summer"], "Dark Winter": ["True Winter", "Dark Autumn"], "Bright Winter": ["True Winter", "Bright Spring"]};
...
function autoDetectSeason() {
  ...
  var labels = {Summer:['Light','True','Soft'],Winter:['Bright','True','Dark'],Autumn:['Soft','True','Dark'],Spring:['Light','True','Bright']};
  groups.forEach(function(g) {
    ...
    labs.forEach(function(label, si) {
      var tally = 0;
      Object.values(votes).forEach(function(v) { if(v===label) tally++; });
      if (tally > 0 && seasons[si]) counts[seasons[si]] = (counts[seasons[si]]||0) + tally;
    });
  });
...
function getTick(pageKey, itemKey, defaultVal) {
  ...
  if (reportTicks[pageKey][itemKey] === undefined) reportTicks[pageKey][itemKey] = defaultVal || false;
...
function previewReport() {
  ...
  try {
    initReportTicks();
  } catch(e) { alert('initReportTicks error: ' + e.message); return; }
  ...
  try {
    var pages = [
      ...
      {n:'Page 8 (Subseason 1)',fn: function(){ return buildSubseasonPage(0,4,8); }},
      {n:'Page 9 (Subseason 2)',fn: function(){ return buildSubseasonPage(0,4,9); }},
      {n:'Page 10 (Warm Seasons)', fn: function(){
        var s=reportSeason, grp='';
        Object.keys(OCA_GROUPS).forEach(function(g){if(OCA_GROUPS[g].indexOf(s)>=0)grp=g;});
        var isW=((REPORT_SEASON_DESC[s]||{}).undertone||'').indexOf('Warm')>=0;
        var wg=isW?[grp,(grp==='Spring'?'Autumn':'Spring')]:['Spring','Autumn'];
        return buildSeasonCompPage(wg,'Warm Seasons','This page shows a comparison of how you look in all the warm seasons.',10);
      }},
      {n:'Page 11 (Cool Seasons)', fn: function(){
        var s=reportSeason, grp='';
        Object.keys(OCA_GROUPS).forEach(function(g){if(OCA_GROUPS[g].indexOf(s)>=0)grp=g;});
        var isW=((REPORT_SEASON_DESC[s]||{}).undertone||'').indexOf('Warm')>=0;
        var cg=isW?['Summer','Winter']:[grp,(grp==='Summer'?'Winter':'Summer')];
        return buildSeasonCompPage(cg,'Cool Seasons','Lastly, I conducted additional draping to confirm your primary season.',11);
      }},
      {n:'Page 12 (Your Season)',  fn: buildPage12},
      {n:'Page 13 (Stripe)',       fn: buildPage13},
      {n:'Page 14 (Palette 1)',    fn: function(){ return buildPalettePage([0,1,2,3],14); }},
      {n:'Page 15 (Palette 2)',    fn: function(){ return buildPalettePage([4,5],15); }},
    ...
    for (var i=0; i<pages.length; i++) {
      try {
        ...
        html += '<div style="background:red;color:white;padding:20px;margin:10px;font-family:monospace">'
          + '<strong>' + pages[i].n + ' ERROR:</strong><br>' + e.message + '<br>' + (e.stack||'').replace(/\n/g,'<br>') + '</div>';
      }
    }
    container.innerHTML = html;
  } catch(e) {
    container.innerHTML = '<div style="background:red;color:white;padding:20px;font-family:monospace"><strong>REPORT ERROR:</strong><br>' + e.message + '</div>';
  }
  document.getElementById('report-preview-title').textContent = name + ' — ' + season;
  document.getElementById('report-preview-overlay').style.display = 'block';
}

function initReportTicks() {
  var isWarm = ((REPORT_SEASON_DESC[reportSeason]||{}).undertone||'').indexOf('Warm')>=0;
  var isCool = !isWarm;
...
function rPage(content) {
  return '<div class="rp-page" style="background:#fff;width:794px;min-height:1050px;border-radius:8px;box-sizing:border-box;font-family:Inter,sans-serif;position:relative;overflow:hidden;padding:32px 40px">'
    + content + '</div>';
}
...
function rStar(pageKey, itemKey, defaultVal) {
  ...
  return '<span id="'+id+'" data-pk="'+pageKey+'" data-ik="'+itemKey+'" class="report-star" style="cursor:pointer;font-size:26px;line-height:1;display:inline-block;user-select:none;color:#FFD700" title="Toggle star">'
    + (active ? '\u2605' : '\u2606') + '</span>';
}
...
function rFaceImg(w, h, extraStyle) {
  ...
  var hs = h ? h+'px' : '100%';
  if (ocaPhoto) {
    return '<img src="'+ocaPhoto+'" style="width:'+ws+';height:'+hs+';object-fit:cover;object-position:center top;display:block;'+(extraStyle||'')+'">';
  }
  return '<div style="width:'+ws+';height:'+hs+';background:#e8e4e0;display:flex;align-items:center;justify-content:center;font-size:22px;'+(extraStyle||'')+'">&#128247;</div>';
}
...
function rStripeBar(season, h) {
  ...
  var ht = h || 50;
  return '<div style="display:flex;overflow:hidden;border-radius:4px;height:'+ht+'px">'
    + cols.map(function(c){ return '<div style="flex:1;background:'+c+'"></div>'; }).join('')
    + '</div>';
}
...
function rColorWheel(season, size) {
  ...
  var cx = sz/2, cy = sz*0.52;
  var rx = sz*0.46, ry = sz*0.5;
  var ir = sz*0.27, iry = sz*0.30;
  var segments = '';
  for (var i=0; i<n; i++) {
    var a1 = (i/n)*2*Math.PI - Math.PI/2;
    var a2 = ((i+1)/n)*2*Math.PI - Math.PI/2;
    var x1o = cx + rx*Math.cos(a1), y1o = cy + ry*Math.sin(a1);
    var x2o = cx + rx*Math.cos(a2), y2o = cy + ry*Math.sin(a2);
    var x1i = cx + ir*Math.cos(a1), y1i = cy + iry*Math.sin(a1);
    var x2i = cx + ir*Math.cos(a2), y2i = cy + iry*Math.sin(a2);
    var la = a2-a1 > Math.PI ? 1 : 0;
    segments += '<path d="M '+x1o+' '+y1o+' A '+rx+' '+ry+' 0 '+la+' 1 '+x2o+' '+y2o
      +' L '+x2i+' '+y2i+' A '+ir+' '+iry+' 0 '+la+' 0 '+x1i+' '+y1i+' Z" fill="'+cols[i]+'"/>';
  }
  var photoHole = '<clipPath id="cp_'+season.replace(/\s/g,'_')+'"><ellipse cx="'+cx+'" cy="'+cy+'" rx="'+(ir-2)+'" ry="'+(iry-2)+'"/></clipPath>'
    + '<image href="'+(ocaPhoto||'')+'" x="'+(cx-ir)+'" y="'+(cy-iry)+'" width="'+(ir*2)+'" height="'+(iry*2)+'" clip-path="url(#cp_'+season.replace(/\s/g,'_')+')" preserveAspectRatio="xMidYMin slice" style="'+(ocaPhoto?'':'display:none')+'"/>';
  return '<svg width="'+sz+'" height="'+sz+'" viewBox="0 0 '+sz+' '+sz+'" style="display:block">'+segments+photoHole+'</svg>';
}
...
function buildPage1() {
  ...
  var swatches = [
    {id:'H1',label:'HAIR'},{id:'E1',label:'EYES'},
    {id:'S1',label:'SKIN 1'},{id:'S2',label:'SKIN 2'},{id:'L1',label:'LIPS'}
  ];
  var swatchHtml = swatches.map(function(s) {
    ...
    return '<div style="width:190px;height:46px;background:'+col+';border-radius:10px;display:flex;align-items:center;padding:0 16px;margin-bottom:10px;box-shadow:0 1px 4px rgba(0,0,0,0.08)">'
      + '<span style="font-size:13px;font-weight:700;letter-spacing:1.5px;color:'+tc+'">'+s.label+'</span>'
      + '</div>';
  }).join('');

  var introText = '<p style="margin:0 0 10px"><strong>In colour analysis, we consider all your features to create a complete picture.</strong> Each feature is assessed carefully, and through a systematic process, we determine your season.</p>'
  ...
  return rPage(
    ...
    + '<div style="font-size:13px;line-height:1.9;text-align:center;margin-bottom:28px;color:#222">'+introText+'</div>'
    + '<div style="display:flex;gap:32px;align-items:flex-end;justify-content:center">'
    + '<div style="flex-shrink:0">'+rFaceImg(260, 320, 'border-radius:4px')+'</div>'
    + '<div style="padding-bottom:8px">'+swatchHtml+'</div>'
    + '</div>'
  );
}
...
function buildPage2() {
  ...
  var grid = rows.map(function(row) {
    ...
    var cells = cols3.map(function(col,ci) {
      var tickKey = row.key+'_'+ci;
      return '<div style="position:relative;width:190px;flex-shrink:0">'
        + '<div style="background:'+col+';border-radius:4px;overflow:hidden;height:210px;position:relative">'
        + rFaceImg(190, 210, 'width:100%;height:210px;object-position:center top')
        + '<div style="position:absolute;top:8px;left:8px;font-size:28px;line-height:1;cursor:pointer;user-select:none" '
        + 'id="tick-skin-'+tickKey+'" data-pk="skin" data-ik="'+tickKey+'" class="report-star" title="Toggle star">'
    ...
    return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">'
      + '<div style="width:40px;flex-shrink:0;font-size:11px;font-weight:700;letter-spacing:1px;writing-mode:vertical-lr;transform:rotate(180deg);text-align:center;color:#333">'+row.label.toUpperCase()+'</div>'
...
function buildPage3() {
  ...
  var range = diff<=3 ? 'Low Contrast' : diff<=6 ? 'Medium Contrast' : 'High Contrast';

  function greyHex(v) {
    var g = Math.round(255 - ((v-1)/9)*210); return 'rgb('+g+','+g+','+g+')';
  }
  function tagBar(label, val) {
    var g = greyHex(val);
    var textDark = val > 5;
    // notch shape using two divs
    return '<div style="display:flex;justify-content:center;align-items:center;margin:20px 0">'
      + '<div style="position:relative;width:220px;height:52px">'
      + '<div style="background:'+g+';border-radius:6px;width:100%;height:100%;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden">'
      ...
      + '<span style="font-size:13px;font-weight:700;color:'+(textDark?'#333':'#ddd')+'">'+val+'</span>'
      + '</div>'
      + '</div>'
      + '</div>';
  }
  ...
  return rPage(
    ...
    + tagBar('HAIR', hairV)
    + tagBar('EYES', eyesV)
    + '<div style="text-align:center;margin-top:40px;font-size:15px;font-weight:700;color:#222">'+range+'</div>'
  );
}
...
function buildPage4() {
  ...
  var cards = pairs.slice(0,8).map(function(p,i) {
    var vote = votes[i] || '';
    ...
    return '<div style="margin-bottom:10px">'
      ...
      + '<div style="flex:1;background:'+p.cool+';position:relative;overflow:hidden">'
      + rFaceImg(0, 120, 'width:100%;height:120px;object-position:center top')
      + '<div style="position:absolute;top:4px;left:4px;font-size:26px;line-height:1;color:#FFD700;text-shadow:0 0 4px rgba(0,0,0,0.5)" id="tick-cw-cool_'+i+'" data-pk="cw" data-ik="cool_'+(i)+'" class="report-star" style="cursor:pointer">'+(coolStar?'★':'')+'</div>'
      + '</div>'
      + '<div style="flex:1;background:'+p.warm+';position:relative;overflow:hidden">'
      + rFaceImg(0, 120, 'width:100%;height:120px;object-position:center top')
      + '<div style="position:absolute;top:4px;left:4px;font-size:26px;line-height:1;color:#FFD700;text-shadow:0 0 4px rgba(0,0,0,0.5)" id="tick-cw-warm_'+i+'" data-pk="cw" data-ik="warm_'+(i)+'" class="report-star" style="cursor:pointer">'+(warmStar?'★':'')+'</div>'
      + '</div>'
      + '</div>'
...
function buildPage5() {
  ...
  return rPage(
    ...
    + '<div style="position:absolute;top:10px;left:10px;font-size:28px;color:#FFD700;text-shadow:0 1px 4px rgba(0,0,0,0.3)" id="tick-blush-cool" data-pk="blush" data-ik="cool" class="report-star" style="cursor:pointer">'+(coolActive?'★':'')+'</div>'
    + '</div>'
    ...
    + '<div style="position:absolute;top:10px;left:10px;font-size:28px;color:#FFD700;text-shadow:0 1px 4px rgba(0,0,0,0.3)" id="tick-blush-warm" data-pk="blush" data-ik="warm" class="report-star" style="cursor:pointer">'+(warmActive?'★':'')+'</div>'
    + '</div>'
...
function buildPage6() {
  ...
  function metalCard(d) {
    ...
    var cardHtml = '<div style="position:relative;height:260px;background:#e8e4e0;overflow:hidden;border-radius:6px">'
      + rFaceImg(0, 260, 'position:absolute;top:0;left:50%;transform:translateX(-50%);height:70%;width:auto;object-position:center top;z-index:1')
      + '<img src="'+(OCA_IMG[d.key]||'')+'" style="position:absolute;left:'+pos.x+'px;top:'+pos.y+'px;width:100%;z-index:2;pointer-events:none">'
      + '</div>'
      + '<div style="text-align:center;padding:8px 0;font-size:13px;font-weight:700;color:#222">'+d.label+'</div>';
    return '<div style="flex:1">'+cardHtml+'</div>';
  }
  var row1 = '<div style="display:flex;gap:20px;margin-bottom:20px">'+metalCard(drapes[0])+metalCard(drapes[1])+'</div>';
  var row2 = '<div style="display:flex;gap:20px">'+metalCard(drapes[2])+metalCard(drapes[3])+'</div>';

  return rPage(
    rHeader('Clear Vs Muted')
    + '<div style="font-size:13px;line-height:1.8;text-align:center;margin-bottom:16px;color:#222">'
    + 'The fourth part of the colour analysis involves draping to refine whether warm, cool, clear, or soft colours suit you best, highlighting the importance of undertone and seasonal variation. For example, shiny gold and silver may harmonise better with your skin than muted metals, revealing your clear quality and preference for vibrant colours over muted tones. This also shows us your best jewellery colours. Here I have put a tick next to your best.'
    + '</div>'
...
function buildPage7() {
  var s = reportSeason;
  var isWarm = ((REPORT_SEASON_DESC[s]||{}).undertone||'').indexOf('Warm')>=0;
  var compSeason = isWarm ? 'True Summer' : 'True Autumn';  // opposite season
  var mainLabel = s + ' ('+((REPORT_SEASON_DESC[s]||{}).tone||'')+' · '+((REPORT_SEASON_DESC[s]||{}).undertone||'')+' Undertone'+')';
  var compLabel = compSeason + ' ('+((REPORT_SEASON_DESC[compSeason]||{}).tone||'')+' · '+((REPORT_SEASON_DESC[compSeason]||{}).undertone||'')+' Undertone'+')';
  var mainGroup = isWarm ? 'COOL muted' : 'WARM';
  var compGroup = isWarm ? 'WARM and muted' : 'COOL muted';
  var desc = 'Below, I&#39;m simply confirming that '+(isWarm?'WARM':'COOL')+' muted tones suit your features better than '+(isWarm?'COOL':'WARM')+' and muted ones. This is also just a nice visual for you.';
  ...
  function seasonBlock(sName, label, isMain) {
    ...
    return '<div style="text-align:center;flex:1">'
      ...
      + '</div>'
      + rStripeBar(sName, 40)
      + '<div style="margin-top:8px;font-size:14px;font-weight:700;color:#222">'+star+' '+sName+'<br><span style="font-size:12px;font-weight:400;color:#666">('+((REPORT_SEASON_DESC[sName]||{}).tone||'')+' · '+((REPORT_SEASON_DESC[sName]||{}).undertone||'')+' Undertone'+')</span></div>'
      + '</div>';
  }

  return rPage(
    rHeader('WARM VS COOL')
    + '<div style="font-size:13px;line-height:1.8;text-align:center;margin-bottom:20px;color:#222">'+desc+'</div>'
...
function buildSubseasonPage(rowStart, rowEnd, pageNum) {
  var s = reportSeason;
  var group = '';
  Object.keys(OCA_GROUPS).forEach(function(g) { if(OCA_GROUPS[g].indexOf(s)>=0) group=g; });
  ...
  var grid = rowSlice.map(function(rowCols, ri) {
    var cells = rowCols.map(function(col, ci) {
      var seasonName = seasons[ci];
      var tickKey = 'sub_'+(rowStart+ri)+'_'+ci;
      // check if this season got votes for this row
      var voted = false;
      Object.keys(subVotes).forEach(function(idx){ if(subVotes[idx]===seasons[ci]) voted=true; });
      return '<div style="position:relative;border-radius:4px;overflow:hidden">'
        + '<div style="background:'+col+';height:170px;position:relative">'
        + rFaceImg(0, 170, 'width:100%;height:170px;object-position:center top')
        + '</div>'
        ...
        + 'id="tick-sub'+pageNum+'-'+tickKey+'" data-pk="sub'+(pageNum)+'" data-ik="'+tickKey+'" class="report-star" title="Toggle star">'
        + (getTick('sub'+pageNum,tickKey,false)?'★':'')+'</div>'
        + '</div>';
    }).join('');
    return '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:10px">'+cells+'</div>';
  }).join('');

  var introText = rowStart===0
  ...
  return rPage(
    ...
    + '<div style="font-size:13px;line-height:1.8;text-align:center;margin-bottom:16px;color:#222">'+introText+'</div>'
    + colHeaders + grid
  );
}
...
function buildSeasonCompPage(groupNames, title, desc, pageNum) {
  var s = reportSeason;
  var seasonGroup = '';
  Object.keys(OCA_GROUPS).forEach(function(g){ if(OCA_GROUPS[g].indexOf(s)>=0) seasonGroup=g; });
  var allSeasons = [];
  groupNames.forEach(function(g){ allSeasons = allSeasons.concat(OCA_GROUPS[g]||[]); });

  function seasonCard(sName) {
    var isGood = (sName===s) || (REPORT_SISTERS[s]||[]).indexOf(sName)>=0;
    var tick = isGood
      ? '<div style="width:36px;height:36px;border-radius:50%;background:#2ecc40;display:flex;align-items:center;justify-content:center;font-size:20px;color:#fff;margin:4px auto 0;cursor:pointer" id="tick-comp'+pageNum+'-'+sName.replace(/\s/g,'')+'" data-pk="comp'+(pageNum)+'" data-ik="'+sName.replace(/\\s/g,'')+'" class="report-star">&#10003;</div>'
      : '<div style="width:36px;height:36px;border-radius:50%;background:#e74c3c;display:flex;align-items:center;justify-content:center;font-size:20px;color:#fff;margin:4px auto 0;cursor:pointer" id="tick-comp'+pageNum+'-'+sName.replace(/\s/g,'')+'" data-pk="comp'+(pageNum)+'" data-ik="'+sName.replace(/\\s/g,'')+'" class="report-star">&#10005;</div>';
  ...
  return rPage(
    rHeader(title)
    + '<div style="font-size:13px;line-height:1.8;text-align:center;margin-bottom:16px;color:#222">'+desc+'</div>'
    + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px">'+row1+'</div>'
    + '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">'+row2+'</div>'
  );
}
...
function buildPage12() {
  ...
  var desc = ((REPORT_SEASON_DESC[s]||{}).tone||'')+' · '+((REPORT_SEASON_DESC[s]||{}).undertone||'')+' Undertone · Contrast: '+((REPORT_SEASON_DESC[s]||{}).contrast||'');
  ...
  function seasonBlock(sName, isMain) {
    return '<div style="text-align:center">'
      ...
      + '<div style="margin-top:6px;font-size:'+(isMain?14:12)+'px;font-weight:700;color:#222">'
      + sName+'<br><span style="font-size:'+(isMain?12:10)+'px;font-weight:400;color:#555">('+((REPORT_SEASON_DESC[sName]||{}).tone||'')+' · '+((REPORT_SEASON_DESC[sName]||{}).undertone||'')+' Undertone'+')</span></div>'
      + '</div>';
  }
  ...
  return rPage(
    ...
    + '</div>'
    + '<div style="font-weight:700;text-align:center;font-size:15px;margin-bottom:16px;color:#222">'+s+' ('+desc+')</div>'
    + '<div style="display:flex;justify-content:center;margin-bottom:20px">'+seasonBlock(s,true)+'</div>'
...
function buildPage13() {
  ...
  var stripeBar = '<div style="display:flex;height:320px;border-radius:6px;overflow:hidden;position:relative">'
    + stripeData.map(function(c){ return '<div style="flex:1;background:'+c+'"></div>'; }).join('')
    + '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center">'
    + rFaceImg(260, 320, 'border-radius:4px;box-shadow:0 2px 16px rgba(0,0,0,0.2)')
    + '</div>'
    + '</div>';
  ...
  return rPage(
    ...
    + stripeBar
    + '<div style="text-align:center;margin-top:16px;font-size:15px;font-weight:700;color:#222">'+s+' ('+desc+')</div>'
  );
}
...
function buildPalettePage(rowGroups, pageNum) {
  ...
  var groups = allGroups.filter(function(g,gi){ return rowGroups.indexOf(gi)>=0; });

  var rows = groups.map(function(g) {
    var cols = g.indices.map(function(i){ return stripeData[i]||'#ccc'; });
    var cells = cols.map(function(col) {
      return '<div style="flex:1;position:relative;border-radius:4px;overflow:hidden">'
        + '<div style="background:'+col+'">'
        + rFaceImg(0, 190, 'width:100%;height:190px;object-position:center top')
        + '</div>'
        + '</div>';
    }).join('');
    return '<div style="display:flex;align-items:center;margin-bottom:12px;gap:6px">'
      + '<div style="width:44px;flex-shrink:0;font-size:9px;font-weight:700;letter-spacing:1px;writing-mode:vertical-lr;transform:rotate(180deg);text-align:center;color:#333">'+g.label+'</div>'
      + '<div style="display:flex;gap:8px;flex:1">'+cells+'</div>'
      + '</div>';
  }).join('');
  ...
  return rPage(
    ...
    + (pageNum===14 ? 'The colours on this page and the next are all part of your season&#39;s palette. However, not all of them may suit you perfectly or align with your personal preferences.' : '')
    + '</div>'
    + rows
  );
}
...
function buildPage16() {
  ...
  var rows = outRows.map(function(row) {
    var cells = row.colours.map(function(col) {
      return '<div style="flex:1;border-radius:4px;overflow:hidden">'
        + '<div style="background:'+col+'">'
        + rFaceImg(0,160,'width:100%;height:160px;object-position:center top')
        + '</div>'
        + '</div>';
    }).join('');
    return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:10px">'
      + '<div style="width:44px;flex-shrink:0;font-size:8px;font-weight:700;letter-spacing:.8px;writing-mode:vertical-lr;transform:rotate(180deg);text-align:center;color:#333">'+row.label+'</div>'
      + '<div style="display:flex;gap:8px;flex:1">'+cells+'</div>'
      + '</div>';
  }).join('');
...
function buildPage17() {
  ...
  return rPage(
    ...
    + '</div>'
    + (img ? '<div style="text-align:center"><img src="'+img+'" style="max-width:100%;max-height:600px;border-radius:6px"></div>' : '<div style="text-align:center;padding:60px;color:#aaa">Pattern image not available</div>')
  );
}
...
function buildPage18() {
  ...
  var html = rows.map(function(row){
    return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:10px">'
      + '<div style="width:52px;flex-shrink:0;font-size:9px;font-weight:700;letter-spacing:.5px;writing-mode:vertical-lr;transform:rotate(180deg);text-align:center;color:#333">'+row.label+'</div>'
  ...
  return rPage(
    rHeader('Pattern & Print to Avoid')
    + html
  );
}
...
function buildPage19() {
  var isWarm = ((REPORT_SEASON_DESC[reportSeason]||{}).undertone||'').indexOf('Warm')>=0;
  var descText = 'See Below where its obvious that a '+(isWarm?'warmer':'cooler')+' lipstick sits better with your complexion. Whilst the '+(isWarm?'cool':'warm')+' one takes the colour from your face and washes you out';
  var footerText = 'You&#39;ll find suggestions and information about makeup in your season guide. It&#39;s incredibly important to try products on before purchasing, even if they fall within your palette, as everyone&#39;s lips and skin can react differently to pigments and formulations. Use the guide as a starting point and go from there.<br><br>Personally, I recommend keeping it simple with just two or three lipsticks—one for everyday wear, one for evening, and possibly one for special occasions. This helps reduce waste and keeps your routine streamlined. All beauty products have expiry dates, so it&#39;s best not to collect more than you&#39;ll use.';
  ...
  function lipPhoto(lipKey) {
    var lipSrc = LIP_OPTIONS ? (LIP_OPTIONS.filter(function(l){return l.key===lipKey;})[0]||{}).src : null;
    var photoSrc = ocaLipPhoto || ocaPhoto;
    if (photoSrc) {
      return '<div style="position:relative;width:220px;height:270px;overflow:hidden;border-radius:4px">'
        + '<img src="'+photoSrc+'" style="width:100%;height:100%;object-fit:cover;object-position:center top">'
        + (lipSrc ? '<img src="'+lipSrc+'" style="position:absolute;bottom:25%;left:50%;transform:translateX(-50%);width:60%;pointer-events:none">' : '')
        + '</div>';
    }
  ...
  return rPage(
    rHeader('Lipsticks')
    + '<div style="font-size:13px;font-weight:700;line-height:1.7;text-align:center;margin-bottom:24px;color:#222">'+descText+'</div>'
    + '<div style="display:flex;justify-content:center;gap:40px;margin-bottom:20px">'
    + '<div style="text-align:center">'+lipPhoto('cool_lip')+'<div style="margin-top:10px;font-size:14px;font-weight:700;color:#222">Cool Lipsticks</div></div>'
    + '<div style="text-align:center">'+lipPhoto('warm_lip')+'<div style="margin-top:10px;font-size:14px;font-weight:700;color:#222">Warm Lipstick</div></div>'
    + '</div>'
    + '<div style="font-size:12px;line-height:1.9;text-align:center;color:#444">'+footerText+'</div>'
  );
}
...
function buildAllReportPages() {
  ...
  return [
    ...
    buildSeasonCompPage(pageWarm, 'Warm Seasons',
      'This page shows a comparison of how you look in all the warm seasons: Spring and Autumn. You&#39;ll notice that only one of these palettes harmonise with your natural features, confirming that warm tones are not suited to your colouring. This is an important step in narrowing down the colours that truly enhance your appearance.',10),
    buildSeasonCompPage(pageCool, 'Cool Seasons',
      'Lastly, I conducted additional draping to confirm your primary season and sister seasons. Below, you&#39;ll find a comparison of how you look in all the cool seasons: Summers and Winters. A tick has been placed next to your identified season and its sister seasons for clarity.',11),
    buildPage12(),
    buildPage13(),
...
function downloadReportPDF() {
  var container = document.getElementById('report-pages-container');
  var btn = document.querySelector('#report-preview-overlay button[onclick="downloadReportPDF()"]');
  if (btn) { btn.textContent = 'Generating PDF...'; btn.disabled = true; }

  var opt = {
    margin:       0,
    filename:     (reportClientName || 'Client') + ' - ' + (reportSeason || 'Season') + ' Report.pdf',
    image:        { type: 'jpeg', quality: 0.95 },
    html2canvas:  { scale: 2, useCORS: true, logging: false },
    jsPDF:        { unit: 'px', format: [794, 1050], orientation: 'portrait', hotfixes: ['px_scaling'] },
  ...
  html2pdf().set(opt).from(container).save().then(function() {
    ...
    if (btn) { btn.textContent = '⬇ Download PDF'; btn.disabled = false; }
    alert('PDF generation failed. Try using Ctrl+P / Cmd+P to print instead.');
  });
}