...
(function () {
  ...
  function v24getTS(task) {
    if (typeof window.getTS === 'function' && typeof window.taskWeekOff !== 'undefined') {
      var result = window.getTS(task, window.taskWeekOff);
      // getTS doesn't return 'notes' (owner notes) — supplement it
      if (result.notes === undefined) result.notes = result.notes || task.notes || '';
      return result;
    }
  ...
  function v24setTS(task, field, value) {
    if (typeof window.setTS === 'function' && typeof window.taskWeekOff !== 'undefined') {
      window.setTS(task, window.taskWeekOff, field, value);
      return;
    }
